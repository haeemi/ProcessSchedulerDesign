...
namespace Schd
{
    class ReadyQueueElement
    {
        ...
        public ReadyQueueElement(int processID, int burstTime, int waitingTime, int arriveTime)
        {
        ...
        public static List<Result> Run(List<Process> jobList, List<Result> resultList)
        {
            ...
            do
            {
                while(jobList.Count != 0)
                {
                    ...
                    {
                        readyQueue.Add(new ReadyQueueElement(frontJob.processID, frontJob.burstTime, 0, runTime));
                        jobList.RemoveAt(0);
                    }
                    else
                    {
                ...
                if (currentProcess == 0)
                {
                    if (readyQueue.Count != 0)
                    {
                        ...
                        resultList.Add(new Result(rq.processID, runTime, rq.burstTime, rq.waitingTime, rq.waitingTime, runTime - rq.arriveTime + rq.burstTime));
